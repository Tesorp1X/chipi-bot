...
func GetItemsList(c tele.Context, state fsm.Context) ([]*static.Item, error) {
	...
	if err := state.Data(context.Background(), static.ITEMS_LIST, &items); err != nil {
		...
		return nil, fmt.Errorf(
			"error in storageHelpers.handleCheckName(): couldn't retrieve items from state-storage (%v). sent with error: %v",
			err,
			sendErr,
		)
	}
	...
	if len(items) == 0 {
		...
		return nil, fmt.Errorf(
			"error in storageHelpers.handleCheckName(): retrieved items-list is empty. sent with error: %v",
			sendErr,
		)
	}
...
func GetCheck(c tele.Context, state fsm.Context) (*static.Check, error) {
	...
	if err := state.Data(context.Background(), static.CHECK, &check); err != nil {
		...
		return nil, fmt.Errorf(
			"error in storageHelpers.GetCheck(): couldn't retrieve check from state-storage (%v). sent with error: %v",
			err,
			sendErr,
		)
	}
...
func GetCurrentIndex(c tele.Context, state fsm.Context) (int, error) {
	...
	if err := state.Data(context.Background(), static.CURRENT_INDEX_ITEMS, &currentIndex); err != nil {
		...
		return -1, fmt.Errorf(
			"error in storageHelpers.GetCurrentIndex(): couldn't retrieve currentIndex from context (%v). send with error: %v",
			err,
			sendErr,
		)
	}
